...
export interface CinematicFooterProps {
  ...
  primaryActions?: ActionItemData[];
  secondaryLinks?: ActionItemData[];
...
const DEFAULT_MARQUEE: MarqueeItemData[] = [
  { label: "Accountability Redefined", symbol: "✦" },
  { label: "Transparent Tracking", symbol: "✦" },
  { label: "12-Step Progress", symbol: "✦" },
  { label: "Sponsor Connection", symbol: "✦" },
  { label: "Absolute Privacy", symbol: "✦" },
];
...
const DEFAULT_PRIMARY_ACTIONS: ActionItemData[] = [
  { label: "Download iOS", href: "#", icon: <AppleIcon /> },
  { label: "Download Android", href: "#", icon: <AndroidIcon /> },
];

const DEFAULT_SECONDARY_LINKS: ActionItemData[] = [
...
export function CinematicFooter({
  ...
  const renderMarqueeCluster = () => (
    <div className="flex items-center space-x-12 px-6">
      {marqueeItems.map((item, idx) => (
        <React.Fragment key={idx}>
          <span>{item.label}</span>
          <span className={idx % 2 === 0 ? "text-primary/60" : "text-secondary/60"}>
            {item.symbol || "✦"}
          </span>
        </React.Fragment>
      ))}
        ...
        <footer className="fixed bottom-0 left-0 flex h-screen w-full flex-col justify-between overflow-hidden bg-background text-foreground cinematic-footer-wrapper">
              ...
              <div className="flex flex-wrap justify-center gap-4 w-full">
                {primaryActions.map((action, idx) => (
                  <MagneticButton
                    ...
                    target={action.isExternal ? "_blank" : action.target}
                    rel={action.isExternal ? "noopener noreferrer" : action.rel}
              ...
              <div className="flex flex-wrap justify-center gap-3 md:gap-6 w-full mt-2">
                {secondaryLinks.map((link, idx) => (
                  <MagneticButton
                    ...
                    target={link.isExternal ? "_blank" : link.target}
                    rel={link.isExternal ? "noopener noreferrer" : link.rel}
          ...
          <div className="relative z-20 w-full pb-8 px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="footer-glass-pill px-6 py-3 rounded-full flex items-center gap-2 order-1 md:order-2 cursor-default border-border/50">
              <span className="text-muted-foreground text-[10px] md:text-xs font-bold uppercase tracking-widest">{authorPrefix}</span>
              ...
              <span className="text-foreground font-black text-xs md:text-sm tracking-normal ml-1">{authorName}</span>
            </div>
